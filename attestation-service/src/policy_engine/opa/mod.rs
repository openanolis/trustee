...
"#
    ...
    fn is_valid_rego_extension_name_rejects_injection_vectors() {
        let rejects = [
            ...
            "crypto.sha256.",
            "crypto..sha256",
            // an injection attempt: a name with a newline + comment that
    ...
    // An injection via the extension name must be blocked at the regovm entry, before
...
"#;
    ...
    // Pins the "skip missing rule" contract: a policy that defines
