...
impl RekorClient {
    ...
    pub async fn fetch_slsa_provenance_for_lookup(&self, lookup: &str) -> Result<Vec<String>> {
        let artifact_hash = sha256_hex(lookup);
        self.fetch_slsa_provenance_by_hash(&artifact_hash).await
    }

    pub async fn fetch_slsa_provenance_by_hash(&self, digest_hex: &str) -> Result<Vec<String>> {
        let uuids = self
            .search_by_hash(digest_hex)
            .await
            .with_context(|| format!("search Rekor index with hash {digest_hex}"))?;

        if uuids.is_empty() {
            bail!("Rekor returned no entries for hash `{digest_hex}`");
        }
